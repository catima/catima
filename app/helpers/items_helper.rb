...
module ItemsHelper
  ...
  def formatted_item_for_timeline(item, list:, container:, filter_field:)
    item.attributes.merge(
      title: item_has_thumbnail?(item) ? tag.div(item_list_link(list, item, 0) { item_thumbnail(item, :class => "media-object") }, class: "pull-left mr-3") : tag.h4(item_list_link(list, item, 0, item_display_name(item)), class: "mt-0 mb-1"),
