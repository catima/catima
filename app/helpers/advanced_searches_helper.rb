...
module AdvancedSearchesHelper
  ...
  #
  def render_advanced_search_field(form, field, index, value=nil, &block)
    ...
    content = form.fields_for(:criteria) do |f|
      f.fields_for(field.uuid) do |f|
        partial_rendered = render(partial, :f => f, :field => field, :i => index, :value => value)
  ...
  def render_and_or_exclude_select(form, selected=nil)
    form.select(
      "field_condition",
      [
        [I18n.t(".and"), "and"],
      ...
      { :hide_label => true, :selected => selected },
