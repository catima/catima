  ...
  def build_complex_fields_relations(items_strategies, strategy, criteria, scope)
    ...
    if strategy.field.is_a?(Field::ComplexDatation)
      criteria = criteria.select do |_k, v|
        ...
        (v[:default]&.length == 0) || (start_or_end_date_present && (v[:start][v[:start].keys.first].each { |_key, value| true if value.present? } && v[:end][v[:end].keys.first].each { |_key, value| true if value.present? }))
      end
    end
